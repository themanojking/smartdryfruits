...
/* NEW: chip used to list what's inside the combo (e.g. "Pumpkin • 100g") */
const SeedChip = styled(Chip)(({ theme }) => ({
    fontFamily: "'Manrope', sans-serif",
    fontWeight: 600,
    fontSize: '0.62rem',
    height: 22,
...
const ProductCard = ({ product, isLoading }) => {
    ...
    const availableGrams = Object.keys(product.prices);
    const defaultGram = availableGrams[0];

    const [selectedGram, setSelectedGram] = useState(defaultGram);
    const [price, setPrice] = useState(product.prices[defaultGram].currentPrice);
    const [originalPrice, setOriginalPrice] = useState(product.prices[defaultGram].originalPrice);
            ...
            <CardContent sx={{
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        {/* NEW: seeds included in the combo, each labelled with its weight */}
                        {Array.isArray(product.contents) && product.contents.length > 0 && (
                            <Stack direction="row" spacing={0.5} flexWrap="wrap" sx={{ mb: 0.8, rowGap: 0.5 }}>
                        ...
                        <Stack direction="row" spacing={0.6} flexWrap="wrap" sx={{ mb: 1.2, rowGap: 0.6 }}>
                            {Object.keys(product.prices).map((gram) => (
                                <GramChip
                                    key={gram}
                                    label={gram}
                                    size="small"
